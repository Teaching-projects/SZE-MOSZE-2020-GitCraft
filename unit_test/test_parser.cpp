...
/*TEST(test_parser, test_filename){
    ...
    std::map<std::string, std::variant<string, int, double>> expected{
        {"name", "Zuli"},
        {"health", "120"},
        {"dmg", "25"},
        {"atc", "5.2"}
    ...
    std::map<std::string, std::variant<string, int, double>> output = JSON::parseFromFile(fileName);
...
TEST(test_parser, test_istream){
    std::map<std::string, std::variant<string, int, double>> output;
    std::map<std::string, std::variant<string, int, double>> expected{
        {"name", "Hamaha"},
        {"health", "100"},
        {"dmg", "35"},
    ...
    output = JSON::loadInputFromFile(unitFile);
    unitFile.close();
...
TEST(test_parser, test_string){
    ...
    file.close();
    output = JSON::loadInput(jsonContent);
...
TEST(test_parser, test_invalid){
    ...
    try{
        JSON::loadInputFromFile(unitFile);   
    }catch(char const* c){
...
TEST(Character_test, getAttackCoolDown){
	Hero ACDtest("Hamaha", 100, 35, 4.2);
...
{
    JSON parsedCA = JSON::parseFromFile("../units/Elf.json");
    JSON parsedCB = JSON::parseFromFile("../units/Orc.json");
    Hero* player = new Hero(parsedCA);
    Monster* enemy = new Monster(parsedCB);

    player->fightTilDeath(enemy);
    ASSERT_EQ(player.getHealthPoints(),121);
}
